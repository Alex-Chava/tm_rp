...
# Берём только те ячейки, где side='2'
hv_cells_all = session.query(HVCell).filter(HVCell.side == "2").all()
session.close()

if not hv_cells_all:
    print("[INFO] Нет hv_cells с side=2, нечего опрашивать.")
    exit(0)
